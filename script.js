document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateLokasiFilter() {
    const uniqueLokasi = [...new Set(allData.slice(1).map(row => row[LOKASI_COLUMN_INDEX]).filter(Boolean))].sort();
    ...
    uniqueLokasi.forEach(lokasi => {
      ...
      option.value = option.textContent = lokasi;
  ...
  function updateCounters(data) {
    ...
    todayCounter.update(validRows.filter(r => { const d = parseTimestamp(r[0]); return d && d >= startOfDay;   }).length);
    monthCounter.update(validRows.filter(r => { const d = parseTimestamp(r[0]); return d && d >= startOfMonth; }).length);
    yearCounter.update( validRows.filter(r => { const d = parseTimestamp(r[0]); return d && d >= startOfYear;  }).length);
  }
  ...
  function filterAndRender() {
    ...
    currentPage = 1;
    filteredData = selectedLokasi === 'all'
      ? allData
      : [allData[0], ...allData.slice(1).filter(row => row[LOKASI_COLUMN_INDEX] === selectedLokasi)];
    renderTable(filteredData);
  ...
  function printToPdf() {
    ...
    doc.text(`Tanggal Cetak: ${today}`, cx, 29, { align: 'center' });
